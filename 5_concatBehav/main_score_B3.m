...
for ID_i = 1:n
    ...
    % block 3
    tmp1.logABDlong(tmp1.logABDlong(:,1)~=3,:)=[];
    tmp2.logABlong(tmp2.logABlong(:,1)~=3,:)=[];
    tmp3.logADlong(tmp3.logADlong(:,1)~=3,:)=[];
    tmp4.logBDlong(tmp4.logBDlong(:,1)~=3,:)=[];
    ...
    % block 3
    tmp1.logABDshort(tmp1.logABDshort(:,1)~=3,:)=[];
    tmp2.logABshort(tmp2.logABshort(:,1)~=3,:)=[];
    tmp3.logADshort(tmp3.logADshort(:,1)~=3,:)=[];
    tmp4.logBDshort(tmp4.logBDshort(:,1)~=3,:)=[];
    ...
    if trials_LH(ID_i,1) ~= 50
        disp(strcat('participant', 32, int2str(ID), 32, 'has less trials in LH:', 32, num2str(trials_LH(ID_i,1)), 32, 'instead of 50'));
    end
    ...
    if trials_SH(ID_i,1) ~= 50
        disp(strcat('participant', 32, int2str(ID), 32, 'has less trials in SH:', 32, num2str(trials_SH(ID_i,1)), 32, 'instead of 50'));
    end
...
save(strcat(dir_save, 'score_block3.mat'), 'score');
