...
for ID_i = 1:n
    ...
    % block 1
    tmp1.logABDlong(tmp1.logABDlong(:,1)~=1,:)=[];
    tmp2.logABlong(tmp2.logABlong(:,1)~=1,:)=[];
    tmp3.logADlong(tmp3.logADlong(:,1)~=1,:)=[];
    tmp4.logBDlong(tmp4.logBDlong(:,1)~=1,:)=[];
    ...
    % block 1
    tmp1.logABDshort(tmp1.logABDshort(:,1)~=1,:)=[];
    tmp2.logABshort(tmp2.logABshort(:,1)~=1,:)=[];
    tmp3.logADshort(tmp3.logADshort(:,1)~=1,:)=[];
    tmp4.logBDshort(tmp4.logBDshort(:,1)~=1,:)=[];
...
save(strcat(dir_save, 'score_block1.mat'), 'score');
