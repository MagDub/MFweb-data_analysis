...
for ID_i = 1:n
    ...
    % block 4
    tmp1.logABDlong(tmp1.logABDlong(:,1)~=4,:)=[];
    tmp2.logABlong(tmp2.logABlong(:,1)~=4,:)=[];
    tmp3.logADlong(tmp3.logADlong(:,1)~=4,:)=[];
    tmp4.logBDlong(tmp4.logBDlong(:,1)~=4,:)=[];
    ...
    % block 4
    tmp1.logABDshort(tmp1.logABDshort(:,1)~=4,:)=[];
    tmp2.logABshort(tmp2.logABshort(:,1)~=4,:)=[];
    tmp3.logADshort(tmp3.logADshort(:,1)~=4,:)=[];
    tmp4.logBDshort(tmp4.logBDshort(:,1)~=4,:)=[];
...
save(strcat(dir_save, 'score_block4.mat'), 'score');
