function [nMAP, nLogL, mo, logL] = modelMF_S0fixed_eta_thomp4param_MAP_2sgm0_etaB(param_val,param_names,sID,settings,data,gameIDs, prior, param_names_prior, blocks)
holly = 0;
...
for c = 1:settings.task.N_hor
    for g = 1:settings.task.Ngames_per_hor
        ...
        tmp_dat = data(c,g);
        tmp_block = blocks(c,g);

        % compute model game
        [logL(c,g),mo] = model_game_MF_2sgm0_B(mo,tmp_dat,tmp_block, c,g,settings.opts.TLT);
