function fit_mod12_xiT(ID, data_fol)
    ...
    settings.params.param_names     = {'sgm0', '', 'Q0', 'eta', '', 'xiT', '', 'xi', ''}; 
    settings.params.lb              = [param_bounds_sgm0(1) param_bounds_sgm0(1)  param_bounds_Q0(1)  param_bounds_eta(1) param_bounds_eta(1) param_bounds_xiT(1) param_bounds_xiT(1) param_bounds_xi(1) param_bounds_xi(1)];    
    settings.params.ub              = [param_bounds_sgm0(2) param_bounds_sgm0(2)  param_bounds_Q0(2)  param_bounds_eta(2) param_bounds_eta(2) param_bounds_xiT(2) param_bounds_xiT(2) param_bounds_xi(2) param_bounds_xi(2)];    
    ...
    % prior
    load(strcat(data_fol,'/priors/mod12_xiT/empirical_prior.mat'),'prior')
    param_names = {'sgm0', 'sgm0', 'Q0','eta', 'eta', 'xiT', 'xiT', 'xi', 'xi'};

    % fit model
    modelfunLL = @(x) modelMF_S0fixed_eta_2sgm0_xiT(x,settings.params.param_names,ID,settings,data,gameIDs,trials);
    modelfun = @(x) modelMF_S0fixed_eta_thomp4param_MAP_2sgm0_xiT(x,settings.params.param_names,ID,settings,data,gameIDs, prior, param_names,trials);
