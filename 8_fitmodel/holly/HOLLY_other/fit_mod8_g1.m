function fit_mod8_g1(ID, data_fol)
    ...
    settings.params.param_names     = {'Q0' 'tau'  ''  'xi'  ''  'eta' ''};   
    settings.params.lb              = [param_bounds_Q0(1) param_bounds_tau(1) param_bounds_tau(1) param_bounds_xi(1) param_bounds_xi(1) param_bounds_eta(1) param_bounds_eta(1)];    % lower bound
    settings.params.ub              = [param_bounds_Q0(2) param_bounds_tau(2) param_bounds_tau(2) param_bounds_xi(2) param_bounds_xi(2) param_bounds_eta(2) param_bounds_eta(2)];    % upper bound
    ...
    param_names = {'Q0', 'tau', 'tau', 'xi', 'xi', 'eta', 'eta'};

    % fit model
    modelfunLL = @(x) modelMF_S0fixed_sgm0fixed_gammafixed_eta(x,settings.params.param_names,ID,settings,data,gameIDs);
    modelfun = @(x) modelMF_S0fixed_sgm0fixed_gammafixed_eta_UCB_MAP(x,settings.params.param_names,ID,settings,data,gameIDs, prior, param_names);
