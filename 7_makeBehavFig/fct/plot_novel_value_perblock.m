...
function [] = plot_novel_value_perblock(to_del)
    ...
    noise_plot = (rand(size(pickedC_SH))-0.5)/10;

    % Short horizon
    b2S = bar(x_ax([3,6,9,12]),nanmean(pickedC_SH),'FaceColor',col_(1,:), 'FaceAlpha', 1, 'BarWidth',0.3); hold on;

    % Long horizon
    b2L = bar(x_ax([4,7,10,13]),nanmean(pickedC_LH),'FaceColor',col_(1,:),'FaceAlpha', 1, 'BarWidth',0.3);

    % error bars
    h = errorbar(x_ax([3 6 9 12 4 7 10 13]),[nanmean(pickedC_SH) nanmean(pickedC_LH)], ...
        [nanstd(pickedC_SH)./sqrt(size(pickedC_SH,1)) nanstd(pickedC_LH)./sqrt(size(pickedC_LH,1))],'.','color','k');
    set(h,'Marker','none')
    
    % lines
    for n = 1:size(pickedC_SH,1)
        lin1 = plot(x_ax([3 4])+noise_plot(n),[pickedC_SH(n,1) pickedC_LH(n,1)]); hold on;
        lin2 = plot(x_ax([6 7])+noise_plot(n),[pickedC_SH(n,2) pickedC_LH(n,2)]); hold on;
        lin3 = plot(x_ax([9 10])+noise_plot(n),[pickedC_SH(n,3) pickedC_LH(n,3)]); hold on;
        lin4 = plot(x_ax([12 13])+noise_plot(n),[pickedC_SH(n,4) pickedC_LH(n,4)]); hold on;
