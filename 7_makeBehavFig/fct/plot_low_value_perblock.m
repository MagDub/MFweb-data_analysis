...
% Short horizon
b2S = bar(x_ax([3,4,5,6]),nanmean(pickedD_SH),'FaceColor',col_(1,:), 'FaceAlpha', 1, 'BarWidth',1);
plot(x_ax([3,4,5,6]).*ones(size(pickedD_SH))+noise_plot, pickedD_SH,'.','MarkerEdgeColor',col_(2,:), 'MarkerSize',2);

% Long horizon
b2L = bar(x_ax([8,9,10,11]),nanmean(pickedD_LH),'FaceColor',col_(1,:),'FaceAlpha', 1, 'BarWidth',1);
plot(x_ax([8,9,10,11]).*ones(size(pickedD_LH))+noise_plot, pickedD_LH,'.','MarkerEdgeColor',col_(2,:), 'MarkerSize',2);

h = errorbar(x_ax([3 4 5 6 8 9 10 11]),[nanmean(pickedD_SH) nanmean(pickedD_LH)], ...
...
% Export
addpath('../../export_fig')
export_fig(['./fig/Fig_behaviour_low_value_perBlock.tif'],'-nocrop','-r200')