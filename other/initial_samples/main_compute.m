...
for n_sim=1:1000
    ...
    for i=1:n
        ...
        elseif tmp_UnusedTree == 3
            initial_samples_meansABD(end+1,:) = [meanA, meanB, meanC, meanD];
            apple1ABD(end+1,:) = tmp_Apple1Size;
        elseif tmp_UnusedTree == 4
            initial_samples_meansABC(end+1,:) = [meanA, meanB, meanC, meanD];
            apple1ABC(end+1,:) = tmp_Apple1Size;
        end

    end

    averageBCD = mean(initial_samples_meansBCD,1);
    averageACD = mean(initial_samples_meansACD,1);
    averageABD = mean(initial_samples_meansABD,1);
    averageABC = mean(initial_samples_meansABC,1);
