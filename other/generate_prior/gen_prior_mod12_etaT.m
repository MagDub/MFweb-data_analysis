...
parameters = {'sgm0'    'Q0'    'xi'    'eta'   'etaT'};
...
for p = 1:length(parameters)
    ...
    elseif ~isempty(strfind(parameters{p},'etaT'))        
        prior(p).dist = 'uniform';
        prior(p).scale = 1;
        prior(p).pd = makedist('Uniform','lower',0,'upper',5);
    else
...
folder_name = ('../../../data/priors/mod12_etaT/');

if ~exist(folder_name)
    mkdir(folder_name)
end

save(strcat(folder_name, 'empirical_prior.mat'),'prior')
