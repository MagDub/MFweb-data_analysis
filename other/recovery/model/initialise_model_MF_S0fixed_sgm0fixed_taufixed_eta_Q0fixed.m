function mo = initialise_model_MF_S0fixed_sgm0fixed_taufixed_eta_Q0fixed(settings)
...
%% set up parameters
mo.params.tau           = 1;   % decision temperature
mo.params.eta           = [];
...
mo.params.sgm0          = 1.5*ones(settings.task.N_games,1); % prior mean std (set up, so that learning can be implemented %1.5
mo.params.Q0            = 5.5*ones(settings.task.N_games,1); % prior mean (set up, so that learning can be implemented
