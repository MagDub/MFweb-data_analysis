...
mean_AIC = nan(n_models,n_models);
std_AIC = nan(n_models,n_models);


for mod=1:n_models

    file_AIC = strcat(sim_fol,'mod',num2str(mod),'_normal/n_sim_',num2str(n_sim),'/results/all_AIC_mat.mat');
    ...
    if exist(file_AIC)
        ...
        % find best models (lowest AIC) for each sim
        [min_val, ind_min_val] = min(tmp.all_AIC_mat');
        min_val = min_val';
        ...
        mean_AIC(mod,:) = mean(tmp.all_AIC_mat,1);
        
        [~, ind_min_val_mean] = nanmin(mean_AIC');
...
t=title('AIC model recovery (N_{sim}=100)','FontSize', 18, 'FontName','Arial', 'Fontweight','normal');
...
% Export
addpath('../../../../export_fig')
export_fig(['./fig/Fig_model_recov_AIC.tif'],'-nocrop','-r200')
