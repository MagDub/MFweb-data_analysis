...
function [settings, data, gameIDs] = sim_mod8_t1_Q0fixed(ID, para_vals, param_bounds, sim_dir)
    ...
    settings.desc                   = ['mod8'];    
    settings.params.param_names     = {'gamma'  ''  'xi'  ''  'eta' ''};   
    settings.params.lb              = [param_bounds.gamma(1) param_bounds.gamma(1) param_bounds.xi(1) param_bounds.xi(1) param_bounds.eta(1) param_bounds.eta(1)];    % lower bound
    settings.params.ub              = [param_bounds.gamma(2) param_bounds.gamma(2) param_bounds.xi(2) param_bounds.xi(2) param_bounds.eta(2) param_bounds.eta(2)];    % upper bound

    % initialise model
    mo = initialise_model_MF_S0fixed_sgm0fixed_taufixed_eta_Q0fixed(settings);
