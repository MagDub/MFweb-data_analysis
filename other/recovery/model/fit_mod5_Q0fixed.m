function fit_mod5_Q0fixed(ID, data_fol, sim_dir, settings, data, gameIDs, param_bounds)
    ...
    settings.params.param_names     = {'gamma'  ''  'tau'  '' };   
    settings.params.lb              = [param_bounds.gamma(1) param_bounds.gamma(1) param_bounds.tau(1) param_bounds.tau(1)];    % lower bound
    settings.params.ub              = [param_bounds.gamma(2) param_bounds.gamma(2) param_bounds.tau(2) param_bounds.tau(2)];    % upper bound

    % prior
    load(strcat(data_fol,'/priors/mod5/empirical_prior.mat'),'prior')
    param_names = {'Q0', 'gamma', 'gamma','tau', 'tau'};

    % fit model
    modelfunLL = @(x) modelMF_S0fixed_sgm0fixed_Q0fixed(x,settings.params.param_names,ID,settings,data,gameIDs);
