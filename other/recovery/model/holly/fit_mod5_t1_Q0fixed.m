function fit_mod5_t1_Q0fixed(ID, data_fol, sim_dir, settings, data, gameIDs, param_bounds)

    results_dir = strcat(sim_dir, 'results/');
    
    % settings
    settings.desc = ['mod5_t1']; 

    settings.funs.decfun            = @softmax;
    settings.funs.valuefun          = @UCB; 
    settings.funs.learningfun       = @kalman_filt;
    settings.params.param_names     = {'gamma'  ''  };   
    settings.params.lb              = [param_bounds.gamma(1) param_bounds.gamma(1)];    % lower bound
    settings.params.ub              = [param_bounds.gamma(2) param_bounds.gamma(2)];    % upper bound

    % prior
    load(strcat(data_fol,'/priors/mod5/empirical_prior.mat'),'prior')
