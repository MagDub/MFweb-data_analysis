function fit_mod6_t1_Q0fixed(ID, data_fol, sim_dir, settings, data, gameIDs, param_bounds)

    results_dir = strcat(sim_dir, 'results/');
    
    % settings
    settings.desc = ['mod6_t1']; 

    settings.funs.decfun            = @softmax;
    settings.funs.valuefun          = @UCB; 
    settings.funs.learningfun       = @kalman_filt;
    settings.params.param_names     = {'gamma'  ''  'xi' ''};   
    settings.params.lb              = [param_bounds.gamma(1) param_bounds.gamma(1) param_bounds.xi(1) param_bounds.xi(1)];    % lower bound
    settings.params.ub              = [param_bounds.gamma(2) param_bounds.gamma(2) param_bounds.xi(2) param_bounds.xi(2)];    % upper bound
    ...
    % fit model
    modelfunLL = @(x) modelMF_S0fixed_sgm0fixed_taufixed_Q0fixed(x,settings.params.param_names,ID,settings,data,gameIDs);
    modelfun = @(x) modelMF_S0fixed_sgm0fixed_taufixed_UCB_MAP_Q0fixed(x,settings.params.param_names,ID,settings,data,gameIDs, prior, param_names);
