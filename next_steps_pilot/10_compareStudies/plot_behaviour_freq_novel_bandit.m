...
NADA_param_SH = [data_NADA.behaviour(:,8)/n_trials*100];
NADA_param_LH = [data_NADA.behaviour(:,4)/n_trials*100];

web_param_SH = [data_web.frequencies(:,3)/n_trials*100];
web_param_LH = [data_web.frequencies(:,7)/n_trials*100];

dev_param_SH = [data_dev.frequencies(:,3)*100/n_trials_dev];
dev_param_LH = [data_dev.frequencies(:,7)*100/n_trials_dev];
...
%% Export
export_fig(['Fig/Fig_novel_bandit.tif'],'-nocrop','-r200')
