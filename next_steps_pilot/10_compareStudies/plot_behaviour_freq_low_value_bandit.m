...
%% Data
data_NADA=load('/Users/magdadubois/MF/figures/NADA/data_for_figs/behaviour.mat');
data_web=load('/Users/magdadubois/MF/data/data_for_figs/frequencies.mat');
data_dev=load('/Users/magdadubois/GoogleDrive/UCL/writing/MF_dev/data_for_figs_2/frequencies.mat');

n_trials = 200;
n_trials_dev = 48;

n_NADA = size(data_NADA.behaviour,1);
n_web = size(data_web.frequencies,1);
n_dev = size(data_dev.frequencies,1);

NADA_param_SH = [data_NADA.behaviour(:,9)/n_trials*100];
NADA_param_LH = [data_NADA.behaviour(:,5)/n_trials*100];

web_param_SH = [data_web.frequencies(:,4)/n_trials*100];
web_param_LH = [data_web.frequencies(:,8)/n_trials*100];

dev_param_SH = [data_dev.frequencies(:,4)*100/n_trials_dev];
dev_param_LH = [data_dev.frequencies(:,8)*100/n_trials_dev];
...
%% Export
export_fig(['Fig/Fig_low_value_bandit.tif'],'-nocrop','-r200')
