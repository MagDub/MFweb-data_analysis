function fit_mod6_t1(ID, data_fol)
    ...
    settings.params.param_names     = {'Q0' 'gamma'  ''  'xi' ''};   
    settings.params.lb              = [param_bounds_Q0(1) param_bounds_gamma(1) param_bounds_gamma(1) param_bounds_xi(1) param_bounds_xi(1)];    % lower bound
    settings.params.ub              = [param_bounds_Q0(2) param_bounds_gamma(2) param_bounds_gamma(2) param_bounds_xi(2) param_bounds_xi(2)];    % upper bound
    ...
    param_names = {'Q0', 'gamma', 'gamma', 'xi', 'xi'};

    % fit model
    modelfunLL = @(x) modelMF_S0fixed_sgm0fixed_taufixed(x,settings.params.param_names,ID,settings,data,gameIDs);
    modelfun = @(x) modelMF_S0fixed_sgm0fixed_taufixed_UCB_MAP(x,settings.params.param_names,ID,settings,data,gameIDs, prior, param_names);
