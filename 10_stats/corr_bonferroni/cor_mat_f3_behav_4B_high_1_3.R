...
cor_mat_f3_behav_4B_high_1_3 <- function() {
    
  load(file = "~/MFweb/data_analysis/10_stats/FA/all_behav_and_FAscores.Rdata")
  
  # keep only mean per horizon
  data_tmp_all <- subset(data_tmp_all, select = -c(3:29, 31:34))
  
  # Choose which ones to keep
  data_all <- data_tmp_all[, c('pickedD_mean', 'pickedC_mean', 'pickedhigh_Aexploit_mean', 'pickedhigh_Bexploit_mean', 'scores_f3')]
  ...
  output=paste(
    ...
    round(df_r$pickedhigh_Aexploit_mean,3),", p_c=", round(df_p_c$pickedhigh_Aexploit_mean,3),", p_u=", round(df_p_u$pickedhigh_Aexploit_mean,3),
    "; F3 and high-value bandit (1 sample): R=", 
    round(df_r$pickedhigh_Bexploit_mean,3),", p_c=", round(df_p_c$pickedhigh_Bexploit_mean,3),", p_u=", round(df_p_u$pickedhigh_Bexploit_mean,3),  
    sep='') 
