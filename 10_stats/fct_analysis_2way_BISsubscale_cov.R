...
rm_anova_MFweb_BISsubscale_cov <- function(x1, x2) {
  ...
  data_tmp_all <- subset(dataMFweb , select=c("User", "exclude", x1, x2, "BIS11_Attentional", "BIS11_Motor", "BIS11_NonPlanning"))
  ...
  res.aov <- anova_test(
    ...
    within = hor,
    covariate = c(BIS11_Attentional, BIS11_Motor, BIS11_NonPlanning),
    effect.size = "pes"
  )
