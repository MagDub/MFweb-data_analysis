...
data_items_d = sapply(data_items, function(data_items) (data_items-mean(data_items)))

# Establish number of factors
CNG <- nCng(cor(data_items_d), model="factors", cor = TRUE, details=TRUE)
number_factors=CNG$nFactors 

# run the factor analysis
fa_results= fa(data_items_d, nfactors = number_factors, rotate = "oblimin", fm="ml")

# compute factor scores
factor_scores = factor.scores(data_items_d, fa_results)
...
write.csv(loadingsplot,'~/MFweb/data_analysis/10_stats/FA/loadingsplot_d.csv')
...
png("~/MFweb/data_analysis/10_stats/FA/FA_loadings_indiv_d.png")
