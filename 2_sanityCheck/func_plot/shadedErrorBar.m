...
edge_strength_param = 0.9; % The bigger, the weaker the color
curve_transparency = 0.1; % The smaller, the weaker the color
...
params.addParameter('patchSaturation', curve_transparency, @(x) isnumeric(x) && x>=0 && x<=1);
...
H = makePlot(x,y,errBar,lineProps,transparent,patchSaturation,edge_strength_param);
...
function H = makePlot(x,y,errBar,lineProps,transparent,patchSaturation,edge_strength_param)
    ...
    edgeColor=mainLineColor+(1-mainLineColor)*edge_strength_param;
